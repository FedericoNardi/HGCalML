...
class LLReduceLoss(LossLayerBase):
    def __init__(self,
                 mode : str = "mse",
                 **kwargs):
        '''
        ...
        super(LLReduceLoss, self).__init__(**kwargs)
        self.mode = mode

    def get_config(self):
        config = {'mode': self.mode}
        base_config = super(LLReduceLoss, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
    
    def loss(self, inputs):
        assert isinstance(inputs, list) and len(inputs) == 2
        model_output, true_values = inputs
        energy_loss = tf.reduce_mean(tf.keras.losses.mean_squared_error(true_values, model_output))
        self.wandb_log({self.name + '_loss': energy_loss})
        return energy_loss # + eta_loss
...
class LLFullObjectCondensation(LossLayerBase):
    ...
    def __init__(self, *, energy_loss_weight=1.,
                 ...
                 potential_scaling=1.,
                 repulsion_scaling=1.,
                 s_b=1., position_loss_weight=1.,
                 classification_loss_weight=1.,
                 timing_loss_weight=1.,
                 use_spectators=True,
                 beta_loss_scale=1.,
    ...
    def calc_timing_loss(self, t_time, pred_time, pred_time_unc, t_dep_energy=None):
        ...
        tloss = tf.math.divide_no_nan(
                (t_time - pred_time)**2 ,
                (pred_time_unc**2 + 1e-1)) + pred_time_unc**2
    ...
    def calc_classification_loss(self,
            orig_t_pid, pred_id, t_is_unique=None, hasunique=None):
...
class LLFullObjectCondensationID(LLFullObjectCondensation):
    ...
    def __init__(self, *args, **kwargs):
        ...
        *
        '''
        super(LLFullObjectCondensationID, self).__init__(*args, **kwargs)
...
class LLExtendedObjectCondensation3(LLExtendedObjectCondensation):
    ...
    def __init__(self, *args, **kwargs):
        super(LLExtendedObjectCondensation3, self).__init__(*args, **kwargs)
...
class LLExtendedObjectCondensation4(LLFullObjectCondensation):
    ...
    def __init__(self, *args, **kwargs):
        super(LLExtendedObjectCondensation4, self).__init__(*args, **kwargs)
...
class LLFullObjectCondensationUncertainty(LLFullObjectCondensation):
    ...
    def calc_energy_correction_factor_loss(self, t_energy, t_dep_energies,
        ...
        uncertainty_loss0 = tf.debugging.check_numerics(
                uncertainty_loss0, "uncertainty_loss0")
        uncertainty_loss1 = tf.debugging.check_numerics(
                uncertainty_loss1, "uncertainty_loss1")
        uncertainty_loss2 = tf.debugging.check_numerics(
                uncertainty_loss2, "uncertainty_loss2")
